...
pack_path = result_path + 'identity' + ('\\tournament' if tourna else '')
...
for card in cards:
    if card['type_code']=='identity' and not(card['title'] in deck_cards_r.columns) and not(card['title'] in deck_cards_c.columns):
        if card['side_code']=='runner':
...
for deck in decks_r:
    for card in cards:
        if card['title'] in [card_cn[x] for x in [*deck['cards']]] and card['type_code']=='identity' and (deck['tournament_badge'] if tourna else True):
